...
List parseMessage(Uint8List msg) {
  ByteData bytes = msg.buffer.asByteData();
  int first_num = bytes.getUint32(0);
  int second_num = bytes.getUint32(4);

  var msg_type = "";

  // print("msg: ${msg.toString()}");
  // print(msg.buffer.lengthInBytes.toString());
  // print("first num: ${first_num.toRadixString(16)}");
  // print("second num: ${second_num.toRadixString(16)}");
  // print("switch: ${((first_num & 0xFF000000) >> 24)}");
...
Uint8List craftMessage(type, name, {num: -1, name2: "dronecone???"}) {
  ...
  if (num != -1) {
    ...
    var num =
        values[3] | (values[2] << 8) | (values[1] << 16) | (values[0] << 24);
    msg_int = msg_int | (num);
  }
